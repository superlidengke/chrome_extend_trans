...
HJ.fun = {
	...
	jsonCallBack : function(json) {
		...
		console.debug(meaning)
		showMeaning(meaning);
		//如果查到单词
		if(!meaning.includes('hjd_noresult')){
			add(wordName, json.content);
			//上传
			$.post("http://localhost/jp/index.php/Home/Word/insert", {name:wordName, meaning: json.content },
					   function(data){
...
var topPt;
	...
	function lookupFromhujiang(param){
	...
	function lookup(param){
		if(param.length>15||param.length<1){
			return;
		}
